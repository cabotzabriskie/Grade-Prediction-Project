...
library(Rmisc)
wk2<-read.csv("Intermediate-Data/Week_2_data.csv", stringsAsFactors = F)
wk2$dup1<-duplicated(wk2$Username, fromLast=F)
wk2<-subset(wk2, !is.na(LabSection))
wk2<-subset(wk2, dup1!=TRUE)

reg<-read.csv("Intermediate-Data/RegistrarData.csv", stringsAsFactors = F)

data<-merge(wk2, reg, by = "Username", all.y = T)
datasave<-data
rm(reg,wk2)
...
sink("Output Text/Univariate wk2 Logistic Models.txt")
...
modL1.1<-glm(train$FinalCourseGradeAB_Rest2 ~ 
               LabQGrade + HWGrade + LabGrade, data = train, family = "binomial"
)
...
write.csv(modL1.2Odds,"Model Fits/Class Wk2 Logistic Fit P2.csv", row.names = T)
...
write.csv(fitLogOdds,"Model Fits/Class and registrar Wk2 Logistic Fit P2.csv", row.names = T)
